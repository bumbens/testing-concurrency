...
import thesis.JCStress.CaseStudies.StringNumbererJCStress.FindTest;
import org.openjdk.jcstress.infra.results.L_Result;

public final class StringNumbererJCStress_FindTest_jcstress extends Runner<L_Result> {

    volatile WorkerSync workerSync;

    public StringNumbererJCStress_FindTest_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private static class JcstressThread_APICheck_writer extends VoidThread {
        ...
        public JcstressThread_APICheck_writer(FindTest t, FindTest s, L_Result r) {
    ...
    private static class JcstressThread_APICheck_reader extends VoidThread {
        ...
        public JcstressThread_APICheck_reader(FindTest t, FindTest s, L_Result r) {
    ...
    private static class JcstressThread_ResourceCheck_writer extends LongThread {
        ...
        public JcstressThread_ResourceCheck_writer(FindTest[] ss, L_Result[] rs, int size) {
        ...
        private void jcstress_check_writer(FindTest[] ls, L_Result[] lr, int size) {
    ...
    private static class JcstressThread_ResourceCheck_reader extends LongThread {
        ...
        public JcstressThread_ResourceCheck_reader(FindTest[] ss, L_Result[] rs, int size) {
        ...
        private void jcstress_check_reader(FindTest[] ls, L_Result[] lr, int size) {
            for (int c = 0; c < size; c++) {
                ls[c].reader(lr[c]);
            }
        }
    ...
    private static class TestResourceEstimator implements ResourceEstimator {
        ...
        public void runWith(int size, long[] cnts) {
            ...
            FindTest[] ls = new FindTest[size];
            L_Result[] lr = new L_Result[size];
            for (int c = 0; c < size; c++) {
                FindTest s = new FindTest();
    ...
    public static void jcstress_ni_consume_final(Counter<L_Result> cnt, FindTest[] ls, L_Result[] lr, FindTest test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
    ...
    public static void jcstress_consume_reinit(Counter<L_Result> cnt, FindTest[] ls, L_Result[] lr, FindTest test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            L_Result r = lr[c];
    ...
    public class JcstressThread_writer extends CounterThread<L_Result> {
        ...
        public JcstressThread_writer(FindTest[] ss, L_Result[] rs, FindTest test) {
    ...
    public class JcstressThread_reader extends CounterThread<L_Result> {
        ...
        public JcstressThread_reader(FindTest[] ss, L_Result[] rs, FindTest test) {
