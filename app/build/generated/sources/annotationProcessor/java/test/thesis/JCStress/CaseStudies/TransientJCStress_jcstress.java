...
import thesis.JCStress.CaseStudies.TransientJCStress;
import org.openjdk.jcstress.infra.results.L_Result;

public final class TransientJCStress_jcstress extends Runner<L_Result> {

    volatile WorkerSync workerSync;

    public TransientJCStress_jcstress(ForkedTestConfig config) {
        super(config);
    }
    ...
    private static class JcstressThread_APICheck_actor1 extends VoidThread {
        ...
        public JcstressThread_APICheck_actor1(TransientJCStress t, TransientJCStress s, L_Result r) {
    ...
    private static class JcstressThread_APICheck_actor2 extends VoidThread {
        ...
        public JcstressThread_APICheck_actor2(TransientJCStress t, TransientJCStress s, L_Result r) {
    ...
    private void jcstress_sanityCheck_API(Counter<L_Result> counter) throws Throwable {
        final TransientJCStress s = new TransientJCStress();
    ...
    private static class JcstressThread_ResourceCheck_actor1 extends LongThread {
        TransientJCStress[] ss;
        L_Result[] rs;
        int size;

        public JcstressThread_ResourceCheck_actor1(TransientJCStress[] ss, L_Result[] rs, int size) {
        ...
        private void jcstress_check_actor1(TransientJCStress[] ls, L_Result[] lr, int size) {
    ...
    private static class JcstressThread_ResourceCheck_actor2 extends LongThread {
        ...
        public JcstressThread_ResourceCheck_actor2(TransientJCStress[] ss, L_Result[] rs, int size) {
        ...
        private void jcstress_check_actor2(TransientJCStress[] ls, L_Result[] lr, int size) {
            for (int c = 0; c < size; c++) {
                ls[c].actor2();
            }
        }
    ...
    private static class TestResourceEstimator implements ResourceEstimator {
        ...
        public void runWith(int size, long[] cnts) {
            ...
            TransientJCStress[] ls = new TransientJCStress[size];
            L_Result[] lr = new L_Result[size];
            for (int c = 0; c < size; c++) {
                TransientJCStress s = new TransientJCStress();
    ...
    public ArrayList<CounterThread<L_Result>> internalRun() {
        ...
        TransientJCStress[] ls = new TransientJCStress[len];
        L_Result[] lr = new L_Result[len];
        for (int c = 0; c < len; c++) {
            ls[c] = new TransientJCStress();
            lr[c] = new L_Result();
        }
    ...
    public static void jcstress_ni_consume_final(Counter<L_Result> cnt, TransientJCStress[] ls, L_Result[] lr, TransientJCStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            L_Result r = lr[c];
            TransientJCStress s = ls[c];
            s.arbiter(r);
            cnt.record(r, 1);
        }
    }

    public static void jcstress_consume_reinit(Counter<L_Result> cnt, TransientJCStress[] ls, L_Result[] lr, TransientJCStress test, int len, int a) {
        int left = a * len / 2;
        int right = (a + 1) * len / 2;
        for (int c = left; c < right; c++) {
            L_Result r = lr[c];
    ...
    public class JcstressThread_actor1 extends CounterThread<L_Result> {
        TransientJCStress[] ss;
        L_Result[] rs;
        TransientJCStress test;

        public JcstressThread_actor1(TransientJCStress[] ss, L_Result[] rs, TransientJCStress test) {
    ...
    public class JcstressThread_actor2 extends CounterThread<L_Result> {
        ...
        public JcstressThread_actor2(TransientJCStress[] ss, L_Result[] rs, TransientJCStress test) {
